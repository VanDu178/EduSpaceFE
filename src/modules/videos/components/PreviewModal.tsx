...
import { Drawer, Switch, Button, Tooltip, Spin } from 'antd';
import { PlayIcon, LockClosedIcon, PencilSquareIcon } from '@heroicons/react/24/outline';
import { SOURCE_TYPE_LABELS, VIDEO_STATUS_COLOR_MAP, VIDEO_STATUS_LABELS, getVideoTypeColors } from '../constants';
import { getHlsPlaylistUrl } from '../utils';
...
export const PreviewModal = ({
  ...
  const { data, isLoading, isFetching } = useVideoDetailQuery(videoId, open, initialVideo);
  const video = data?.video || initialVideo || null;
  ...
  const handleTimeUpdate = () => {
    if (video && isTeaserMode && video.isPremium && videoRef.current) {
      if (!video.teaserDuration || video.teaserDuration <= 0) {
        videoRef.current.pause();
        setIsTeaserLimitReached(true);
          ...
          <div className="flex items-center gap-3">
            {video && video.isPremium && (
              <div className="flex items-center gap-2 bg-slate-100/80 px-3 py-1.5 rounded-xl border border-slate-200/80">
                ...
                >
                  {isTeaserMode ? 'Xem thử' : 'Xem đầy đủ'}
                </span>
                |
    ...
    >
      {!video && (isLoading || isFetching) ? (
        <div className="py-20 flex flex-col items-center justify-center gap-3">
          <Spin size="large" />
          <span className="text-slate-500 text-xs font-medium">Đang tải chi tiết video...</span>
        </div>
      ) : !video ? (
              ...
              {video.sourceType === 'youtube' && video.youtubeVideoId ? (
                ...
                <HlsPlayer
                  ...
                  src={getHlsPlaylistUrl(
                    video.storagePath?.startsWith('bunny://') || video.storagePath?.startsWith('http')
                      ? video.storagePath
                      : video.id
                  )}
              ...
              <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-md flex flex-col items-center justify-center text-white p-6 text-center z-20">
                ...
                <h4 className="text-base font-bold text-white mb-1">
                  {video.teaserDuration && video.teaserDuration > 0
                    ? 'Đã hết thời gian xem thử'
                    : 'Nội dung video cần trả phí'}
                </h4>
