...
import { PAYMENT_ICON_OPTIONS } from '../constants';
...
const FormUpdate = ({ visible, data, onCancel, onSubmit, loading, existingMethods = [] }: FormUpdateProps) => {
  ...
  const otherIcons = existingMethods
    .filter((m) => m.id !== data?.id)
    .map((m) => m.icon);

  const iconOptions = PAYMENT_ICON_OPTIONS.map((opt) => {
    const isIconUsed = otherIcons.includes(opt.value);
    return {
      value: opt.value,
      label: (
